...
namespace CreatureScriptsParser
{
    public static class Sql
    {
        public static DataSet WorldDatabaseSelectQuery(string query)
        {
            ...
            sqlConnection.ConnectionString = "server = localhost;" + " port = 3306;" + " user id = root;" + " password = root;" + " database = world_dragonflight;";
            try
            {
                sqlConnection.Open();
        ...
        public static DataSet HotfixDatabaseSelectQuery(string query)
        {
            DataSet dataSet = new DataSet();
            MySqlConnection sqlConnection = new MySqlConnection();
            sqlConnection.ConnectionString = "server = localhost;" + " port = 3306;" + " user id = root;" + " password = root;" + " database = hotfix_dragonflight;";
            try
            {
                sqlConnection.Open();
                MySqlCommand myCommand = new MySqlCommand(query, sqlConnection);
